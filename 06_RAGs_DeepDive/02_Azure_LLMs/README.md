...
chat_model = AzureChatOpenAI(
    azure_endpoint=azure_openai_endpoint,  # Long URI, not the base
...
The example shown in the video is the following (but it's not really used):
...
#  AZURE_CONTAINER_RG_NAME: demo-container
...
embeddings = AzureOpenAIEmbeddings(
    ...
    azure_endpoint=azure_openai_endpoint,  # Short URL or base
...
The final example code and explanation is in [mxagar/azure-rag-app](https://github.com/mxagar/azure-rag-app).
...
The application consists of a FastAPI instance which can run the RAG pipeline. This FastAPI application is deployed as an Azure Container App.

Prior to the application deployment, all the necessary Azure services need to be deployed and their variables/secrets need to be noted:

- Azure AI Search: vectorstore where documents are indexed.
- Azure OpenAI models: the chat and embedding models.

The deployment of the FastAPI application occurs following these steps:
...
For more details, check [mxagar/azure-rag-app](https://github.com/mxagar/azure-rag-app).
